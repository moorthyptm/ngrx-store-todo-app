...
export function TodoReducer(state: ITodoListModel = initialState, action: TodoActions.TodoActions) {
    switch (action.type) {
        case TodoActions.ADD:
            return {
                ...state,
                data: [...state.data, new TodoList(action.payload.title, action.payload.comment)]
            };
        case TodoActions.EDIT_START:
            return {
                ...state, editIndex: action.payload, editState: true
            };
